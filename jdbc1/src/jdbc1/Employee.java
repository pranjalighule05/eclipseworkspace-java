    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            ResultSet rs_pune = st.executeQuery(
                    "SELECT name FROM employee_data WHERE city='Pune'"
            );

            ArrayList<String> puneList = new ArrayList<>();
            while (rs_pune.next()) {
                puneList.add(rs_pune.getString("name"));
            }
            System.out.println("\nEmployees from Pune: " + puneList);

            
            ResultSet rs_java = st.executeQuery(
                    "SELECT name FROM employee_data WHERE department='Java'"
            );

            ArrayList<String> javaList = new ArrayList<>();
            while (rs_java.next()) {
                javaList.add(rs_java.getString("name"));
            }
